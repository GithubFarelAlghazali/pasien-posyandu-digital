...
async function requestFirestore(url: string, options?: RequestInit) {
  if (!PROJECT_ID || !API_KEY) {
    throw new Error(
      'Firebase env belum lengkap. Cek VITE_FIREBASE_PROJECT_ID dan VITE_FIREBASE_API_KEY di file .env.'
    );
  }
...
export async function findPasienByLogin(
  usernameOrEmail: string,
  password: string
) {
  const usersByNama = await queryDocuments<PasienUser>(
    'users',
    'nama',
    'EQUAL',
    usernameOrEmail
  ...
  const usersByEmail = await queryDocuments<PasienUser>(
    'users',
    'email',
    'EQUAL',
    usernameOrEmail
  ...
  const users = [...usersByNama, ...usersByEmail];

  return (
    users.find(
      (user) => user.role === 'pasien' && user.password === password
    ) || null
  );
}

export async function registerPasien(data: PasienUser) {
  const sameNik = await queryDocuments<PasienUser>(
    'users',
    'nik',
    'EQUAL',
    data.nik
  ...
  const sameEmail = await queryDocuments<PasienUser>(
    'users',
    'email',
    'EQUAL',
    data.email
  ...
  const sameNama = await queryDocuments<PasienUser>(
    'users',
    'nama',
    'EQUAL',
    data.nama
